...
export async function GET(request: NextRequest) {
  ...
  const tools = [
    ...
    tool('create_designs', 'POST', '/api/bot/designs', 'Create/update designs in bulk (coil AND base).', { designs: { type: 'array', description: 'array of { externalId?, id?, name, description?, tags?, status?, coilOnly?, coilImageUrl?, baseImageUrl?, collection?, coilDimensions?, baseDimensions? }' } }, ['designs']),
    tool('get_design', 'GET', '/api/bot/design', 'Full deep view of ONE design: specs, all comments, complete performance history.', idRef),
    tool('generate_artwork', 'POST', '/api/bot/designs/generate', 'Generate real etch artwork (coil AND/OR base) onto a design.', { ...idRef, part: { type: 'string', enum: ['coil', 'base', 'both'] }, prompt: { type: 'string' }, coilPrompt: { type: 'string' }, basePrompt: { type: 'string' }, baseShape: { type: 'string', enum: ['circle', 'oval', 'square', 'rectangle'] }, size: { type: 'string', enum: ['1024x1024', '1536x1024', '1024x1536'] } }),
    tool('generate_variations', 'POST', '/api/bot/designs/variations', 'Generate several alternative takes on a design to choose from.', { ...idRef, part: { type: 'string', enum: ['coil', 'base'] }, count: { type: 'number' }, prompt: { type: 'string' }, applyIndex: { type: 'number' }, size: { type: 'string', enum: ['1024x1024', '1536x1024', '1024x1536'] } }),
    ...
    tool('run_autopilot', 'POST', '/api/bot/autopilot', 'Daily design engine: study what sells + upcoming events, create N on-trend designs, optionally generate coil/base/both artwork. Run once a day.', { count: { type: 'number' }, generateArt: { type: 'boolean' }, part: { type: 'string', enum: ['coil', 'base', 'both'] }, size: { type: 'string', enum: ['1024x1024', '1536x1024', '1024x1536'] }, theme: { type: 'string' }, eventWindowDays: { type: 'number' } }),
    tool('organize_designs', 'POST', '/api/bot/organize', 'Bulk-organize: move designs into collections and add/remove/replace tags.', { items: { type: 'array', description: 'array of { conceptId?|externalId?, collection?, addTags?, removeTags?, setTags?, coilOnly? }' } }, ['items']),
    tool('get_tags', 'GET', '/api/bot/tags', 'The catalog\'s tag vocabulary with usage counts.', { limit: { type: 'number' } }),
    tool('get_collections', 'GET', '/api/bot/collections', 'Every collection with design counts + status breakdown.', {}),
